...
import { LoginCodeValidation } from "@/lib/validations/validation";
import CustomFormField, { FormFieldType } from "../CustomFormField";
import Button from "../Button";

export const LoginDoctorForm = () => {
  ...
  const onSubmit = async (values: z.infer<typeof LoginCodeValidation>) => {
    ...
    axios.post('/api/loginDoctor', values)
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="flex-1 space-y-6 mt-10">
        ...
        />
         <div className="flex justify-between mt-5">
          ...
          <div className="text-center mt-4 text-[#3d2d5c9a]">Don't have an account? <span className="text-[#FFC000] font-semibold cursor-pointer hover:underline " onClick={()=> router.push('/registerDoctor')}>Sign up</span></div>
      </form>
    </Form>
  );
};